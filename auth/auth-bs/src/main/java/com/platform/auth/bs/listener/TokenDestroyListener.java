package com.platform.auth.bs.listener;
...
public class TokenDestroyListener implements ChannelAwareMessageListener {
    ...
    @Autowired
    private IAccessTokenEntityService accessTokenEntityService;
    ...
    private TokenDestoryModel parseMessage(Message message){
        ...
        try{
            log.info("收到销毁token消息,内容:{}", content);
     ...
     * @param mdoel
     */
    private void check(TokenDestoryModel mdoel){
        if(null == mdoel){
            // 抛特定异常
            ...
            if (CollectionUtils.isNotEmpty(idUsers)){
                for(String idUser : idUsers){
                    if (StringUtils.isNotBlank(idUser)){
                        accessTokenEntityService.updateTokenToInvalidByIdUser(idUser);
                    }
                }
            }
            // 销毁指定的token
            List<String> idTokens = model.getIdTokens();
            if (CollectionUtils.isNotEmpty(idTokens)){
                accessTokenEntityService.updateToInvalid(idTokens);
            }
            ...
            if (null !=  userTokenMap){
                for(Entry<String, List<String>> entry: userTokenMap.entrySet()){
                    if (CollectionUtils.isNotEmpty(entry.getValue())){
                        accessTokenEntityService.updateToInvalid(entry.getValue(), entry.getKey());
                    }
                }
            }
